...
static inline UINT GetButtonType(LONG window_style)
{
    return (window_style & BS_TYPEMASK);
}

/* retrieve the button text; returned buffer must be freed by caller */
static inline WCHAR *GetButtonText(const BUTTON_INFO *infoPtr)
{
...
static UINT BUTTON_BStoDT(DWORD style, DWORD ex_style)
{
    ...
    switch (style & BS_CENTER)
    {
        ...
        default:
            ...
            if (GetButtonType(style) <= BS_DEFPUSHBUTTON) dtStyle |= DT_CENTER;
     ...
     */
    if (GetButtonType(style) != BS_GROUPBOX)
    {
...
static inline void PaintButton(BUTTON_INFO *infoPtr, LONG style, UINT action)
{
    HTHEME theme = GetWindowTheme(infoPtr->hwnd);
    RECT rc;
    HDC hdc = GetDC(infoPtr->hwnd);
    /* GetDC appears to give a dc with a clip rect that includes the whole parent, not sure if it is correct or not. */
    GetClientRect(infoPtr->hwnd, &rc);
    IntersectClipRect (hdc, rc.left, rc. top, rc.right, rc.bottom);

    if ((!theme && BUTTON_PaintWithTheme(theme, infoPtr, hdc, 0)) && (btnPaintFunc[style] && IsWindowVisible(infoPtr->hwnd)))
...
static BOOL button_update_uistate(BUTTON_INFO *infoPtr)
{
    ...
    if ((LONG)infoPtr->ui_state != flags)
    {
...
static LRESULT CALLBACK BUTTON_WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    UINT btn_type = GetButtonType(style);
    ...
    switch (uMsg)
    {
        case WM_GETDLGCODE:
            switch(btn_type)
            {
                ...
                case BS_SPLITBUTTON:
                    return DLGC_BUTTON | DLGC_UNDEFPUSHBUTTON | DLGC_WANTARROWS;
        ...
        case WM_ENABLE:
                PaintButton(infoPtr, btn_type, ODA_DRAWENTIRE);
            break;
        ...
        case WM_SETFOCUS:
            ...
            if (btn_type != BS_OWNERDRAW)
                ...
                PaintButton(infoPtr, btn_type, ODA_FOCUS);
...
static UINT BUTTON_CalcLabelRect(const BUTTON_INFO *infoPtr, HDC hdc, RECT *rc)
{
   ...
   switch (style & (BS_ICON|BS_BITMAP))
   {
      case BS_TEXT:
      {
          ...
          if (!(text = GetButtonText(infoPtr))) goto empty_rect;
          if (!text[0])
          {
              heap_free(text);
              goto empty_rect;
...
static void PB_Paint(const BUTTON_INFO *infoPtr, HDC hDC, UINT action)
{
    ...
    if (GetButtonType(style) == BS_DEFPUSHBUTTON)
    {
        if (action != ODA_FOCUS)
        {
...
draw_focus:
    if ((action == ODA_FOCUS || (state & BST_FOCUS)) &&
        !(infoPtr->ui_state & UISF_HIDEFOCUS))
    {
        InflateRect(&rc, -2, -2);
...
static void CB_Paint(const BUTTON_INFO *infoPtr, HDC hDC, UINT action)
{
    ...
    if (action == ODA_DRAWENTIRE || action == ODA_SELECT)
    {
        ...
        if ((GetButtonType(style) == BS_RADIOBUTTON) ||
            (GetButtonType(style) == BS_AUTORADIOBUTTON))
...
static void PB_ThemedPaint(HTHEME theme, const BUTTON_INFO *infoPtr, HDC hDC, ButtonState drawState, UINT dtFlags, BOOL focused, LPARAM prfFlag)
{
    ...
    WCHAR *text = GetButtonText(infoPtr);
    HWND parent;
    DWORD cdrf;
...
static void CB_ThemedPaint(HTHEME theme, const BUTTON_INFO *infoPtr, HDC hDC, ButtonState drawState, UINT dtFlags, BOOL focused, LPARAM prfFlag)
{
    ...
    UINT btn_type = GetButtonType(dwStyle);
