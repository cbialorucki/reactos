...
{
    assert(i >= 0 && i < (INT)infoPtr->uNumItem);
...
static LRESULT TAB_SetCurFocus (TAB_INFO *infoPtr, INT iItem)
{
  ...
  else if (iItem < (INT)infoPtr->uNumItem) {
  ...
  RECT*       selectedRect)
{
  ...
  if ( (infoPtr->uNumItem <= 0) ||
       (itemIndex >= (INT)infoPtr->uNumItem) ||
...
static LRESULT TAB_KeyDown(TAB_INFO* infoPtr, WPARAM keyCode, LPARAM lParam)
{
  ...
  if (newItem >= 0 && newItem < (INT)infoPtr->uNumItem && infoPtr->uFocus != newItem)
...
static INT TAB_InternalHitTest (const TAB_INFO *infoPtr, POINT pt, UINT *flags)
{
  ...
  for (iCount = 0; iCount < (INT)infoPtr->uNumItem; iCount++)
  {
...
TAB_LButtonDown (TAB_INFO *infoPtr, WPARAM wParam, LPARAM lParam)
{
  ...
  if ((newItem != -1) && (infoPtr->iSelected != newItem))
  {
    ...
    else
    {
      ...
      /* any button pressed ? */
      for (i = 0; i < (INT)infoPtr->uNumItem; i++)
  ...
  const RECT* clientRect)
{
  ...
  if (infoPtr->needsScrolling)
  {
    ...
    if(infoPtr->uNumItem)
    {
       ...
       if(maxRange == (INT)infoPtr->uNumItem)
          maxRange--;
    }
  }
  else
...
static void TAB_SetItemBounds (TAB_INFO *infoPtr)
{
  ...
  /* Arrange all tabs evenly if style says so */
       ...
       (infoPtr->uNumRows > 1))
   {
	...
	INT iCount=0;
        ...
        while(iIndexStart < (INT)infoPtr->uNumItem)
        {
          ...
          for (iIndexEnd=iIndexStart;
              (iIndexEnd < (INT)infoPtr->uNumItem) &&
  ...
  {
    RECT rcOriginal;
    for(iIndex = 0; iIndex < (INT)infoPtr->uNumItem; iIndex++)
    {
...
/*  if (drawRect == NULL) */
  ...
  */
  ...
  if ((infoPtr->dwStyle & TCS_OWNERDRAWFIXED) && IsWindow(infoPtr->hwndNotify))
  {
    ...
    if (infoPtr->cbInfo > (INT)sizeof(LPARAM))
  ...
  if (isVisible)
  {
	...
	clBottom = rect.bottom;
          ...
          && ((infoPtr->dwStyle & (TCS_VERTICAL | TCS_BOTTOM)) == 0))
      {
          ...
          if (TAB_GetItem(infoPtr, iItem)->rect.top == (LONG)infoPtr->uNumRows-1)
            r.bottom += 3;
