 ...
 * PURPOSE:     Dynamic pointer array implementation (DPA)
...
extern "C" HRESULT WINAPI DPA_LoadStream(HDPA *phDpa, PFNDPASTREAM loadProc, IStream *pStream, LPVOID pData)
{
    ...
    *phDpa = NULL;
    ...
    errCode = pStream->Seek(position, STREAM_SEEK_CUR, &initial_pos);
    if (errCode != S_OK)
        return errCode;
                                         ...
                                         IStream *pStream, LPVOID pData)
{
    ...
    position.QuadPart = 0;
    hr = pStream->Seek(position, STREAM_SEEK_CUR, &initial_pos);
    if (hr != S_OK)
        return hr;
    ...
    position.QuadPart = 0;
    pStream->Seek(position, STREAM_SEEK_CUR, &curr_pos);
    ...
    position.QuadPart = initial_pos.QuadPart;
    pStream->Seek(position, STREAM_SEEK_SET, NULL);
    pStream->Write(&streamData, sizeof(streamData), NULL);
