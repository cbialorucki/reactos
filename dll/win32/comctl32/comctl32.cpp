...
static WCHAR* GetManifestPath(BOOL create, BOOL bV6)
{
    ...
    pwszBuf = (WCHAR*)HeapAlloc(GetProcessHeap(), 0, MAX_PATH * sizeof(WCHAR));
    if (!pwszBuf)
        return NULL;
...
HWND WINAPI CreateToolbarEx (HWND hwnd, DWORD style, UINT wID, INT nBitmaps, HINSTANCE hBMInst,
