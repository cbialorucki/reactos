...
static int round_child_height(const REBAR_BAND *lpBand, int cyHeight)
{
    ...
    cy = min((UINT)cy, lpBand->cyMaxChild);
    return cy;
}
...
REBAR_DrawBand (HDC hdc, const REBAR_INFO *infoPtr, REBAR_BAND *lpBand)
{
    ...
    {
        new1 = (lpBand->clrFore == CLR_DEFAULT) ? infoPtr->clrBtnText : lpBand->clrFore;
        oldcolor = SetTextColor (hdc, new1);
    }
     ...
     /* *** Supports only Horizontal bars. ***                   */
{
    ...
    for(i=rstart; i<rend; i++){
      ...
      if ((lpBand->fStyle & RBBS_USECHEVRON) && ((UINT)(lpBand->rcChild.right - lpBand->rcChild.left) < lpBand->cxIdeal))
      {
...
{
    int iRow = REBAR_GetBand(infoPtr, iBand)->iRow;
    while ((UINT)(iBand = next_visible(infoPtr, iBand)) < infoPtr->uNumBands)
        if (REBAR_GetBand(infoPtr, iBand)->iRow != iRow)
            break;
    return iBand;
}
...
static REBAR_BAND *REBAR_FindBandToGrow(const REBAR_INFO *infoPtr, INT iBeginBand, INT iEndBand)
{
    ...
    if (i < iBeginBand)
        for (i = prev_visible(infoPtr, iEndBand); i >= iBeginBand; i = prev_visible(infoPtr, i))
            if (REBAR_GetBand(infoPtr, i)->cxMinBand == (UINT)cxMinFirstBand)
                break;
...
static int REBAR_ShrinkBandsRTL(const REBAR_INFO *infoPtr, INT iBeginBand, INT iEndBand, INT cxShrink, BOOL bEnforce)
{
    ...
    for (i = prev_visible(infoPtr, iEndBand); i >= iBeginBand; i = prev_visible(infoPtr, i))
    {
        ...
        if (bEnforce && lpBand->cx > (UINT)lpBand->cxEffective)
    ...
    INT iLastBand, INT xOff, BOOL reorder)
{
    ...
    else
    {
        ...
        for (i = iFirstBand; i < iLastBand; i = next_visible(infoPtr, i))
        {
            ...
            if((UINT)xPos + band->cxMinBand / 2 > (UINT)xOff)
                break;
...
static int REBAR_SetBandsHeight(const REBAR_INFO *infoPtr, INT iBeginBand, INT iEndBand, INT yStart)
{
    ...
    for (i = iBeginBand; i < iEndBand; i = next_visible(infoPtr, i))
    {
        ...
        yMaxHeight = max((UINT)yMaxHeight, lpBand->cyMinBand);
    }
...
REBAR_Layout(REBAR_INFO *infoPtr)
{
    ...
    /* divide rows */
    for (i = rowstart; (UINT)i < infoPtr->uNumBands; i = next_visible(infoPtr, i))
    {
        lpBand = REBAR_GetBand(infoPtr, i);

        if (i > rowstart && (lpBand->fStyle & RBBS_BREAK || xMin + lpBand->cxMinBand > (UINT)adjcx)) {
    ...
    if ((UINT)rowstart < infoPtr->uNumBands)
...
REBAR_SizeChildrenToHeight(const REBAR_INFO *infoPtr, int iBeginBand, int iEndBand, int extra, BOOL *fChanged)
{
    ...
    for (i = iBeginBand; i < iEndBand; i = next_visible(infoPtr, i))
    {
        ...
        if (lpBand->hwndChild && (UINT)cyChild != lpBand->cyChild && (lpBand->fStyle & RBBS_VARIABLEHEIGHT))
        {
        ...
        cyBandsNew = max((UINT)cyBandsNew, lpBand->cyMinBand);
    }
...
REBAR_SizeToHeight(REBAR_INFO *infoPtr, int height)
{
    ...
    if (infoPtr->dwStyle & RBS_VARHEIGHT)
    {
        ...
        int iRow = 0;
        while ((UINT)i < infoPtr->uNumBands)
        {
            REBAR_BAND *lpBand = REBAR_GetBand(infoPtr, i);
            ...
            for (rowEnd = next_visible(infoPtr, i); (UINT)rowEnd < infoPtr->uNumBands; rowEnd = next_visible(infoPtr, rowEnd))
     ...
     /*  separators are drawn on the first band of the next row.    */
{
    ...
    if (lpBand->clrBack != CLR_NONE) {
        new1 = (lpBand->clrBack == CLR_DEFAULT) ? infoPtr->clrBtnFace :
            lpBand->clrBack;
#if GLATESTING
...
REBAR_HandleUDDrag (REBAR_INFO *infoPtr, const POINT *ptsmove)
{
    ...
    if((hitBand->fStyle & RBBS_BREAK) &&
        ((UINT)iHitBand < infoPtr->uNumBands - 1))
    ...
    else if(yOff > REBAR_GetBand(infoPtr, infoPtr->uNumBands - 1)->rcBand.bottom)
    {
        ...
        if((UINT)iHitBand == infoPtr->uNumBands-1 && hitBand->fStyle&RBBS_BREAK)
            return;
        DPA_DeletePtr(infoPtr->bands, iHitBand);
        hitBand->fStyle |= RBBS_BREAK;
        infoPtr->iGrabbedBand = DPA_InsertPtr(
    ...
    else
    {
        ...
        while((UINT)iRowBegin < infoPtr->uNumBands)
        {
...
static LRESULT REBAR_InsertBandT(REBAR_INFO *infoPtr, INT iIndex, const REBARBANDINFOW *lprbbi, BOOL bUnicode)
{
    ...
    if (!(lpBand = (REBAR_BAND*)Alloc(sizeof(REBAR_BAND))))
        return FALSE;
    if ((iIndex == -1) || ((UINT)iIndex > infoPtr->uNumBands))
...
static LRESULT REBAR_MaximizeBand(const REBAR_INFO *infoPtr, INT iBand, LPARAM lParam)
{
    ...
    /* Validate */
    if (infoPtr->uNumBands == 0 || iBand < 0 || (UINT)iBand >= infoPtr->uNumBands)
    {
        /* error !!! */
...
static LRESULT REBAR_MinimizeBand(const REBAR_INFO *infoPtr, INT iBand)
{
    ...
    if (iPrev < 0 || REBAR_GetBand(infoPtr, iPrev)->iRow != lpBand->iRow)
    {
        ...
        if ((UINT)iNext < infoPtr->uNumBands && REBAR_GetBand(infoPtr, iNext)->iRow == lpBand->iRow)
        {
...
static LRESULT REBAR_MoveBand (REBAR_INFO *infoPtr, INT iFrom, INT iTo)
{
    ...
    /* Validate */
    if ((infoPtr->uNumBands == 0) ||
    (iFrom < 0) || (UINT)iFrom >= infoPtr->uNumBands ||
    (iTo < 0)   || (UINT)iTo >= infoPtr->uNumBands)
    {
        /* error !!! */
...
static LRESULT REBAR_SetBandInfoT(REBAR_INFO *infoPtr, INT iBand, const REBARBANDINFOW *lprbbi, BOOL bUnicode)
{
    ...
    if ((UINT)iBand >= infoPtr->uNumBands)
        return FALSE;
...
static inline LRESULT REBAR_SetUnicodeFormat(REBAR_INFO *infoPtr, BOOL unicode)
{
...
static LRESULT REBAR_SizeToRect(REBAR_INFO *infoPtr, WPARAM flags, RECT *lpRect)
{
...
static LRESULT REBAR_MouseMove(REBAR_INFO *infoPtr, LPARAM lParam)
{
    ...
    if (infoPtr->iGrabbedBand >= 0)
    {
        ...
        if ((abs(ptMove.x - infoPtr->dragNow.x) <= mindragx) &&
            (abs(ptMove.y - infoPtr->dragNow.y) <= mindragy))
            return 0;
...
static inline LRESULT REBAR_NCCalcSize (const REBAR_INFO *infoPtr, RECT *rect)
{
    HTHEME theme;
...
static LRESULT REBAR_NCCreate (HWND hwnd, const CREATESTRUCTW *cs)
{
    ...
    infoPtr->fStatus = 0;
    infoPtr->hFont = (HFONT)GetStockObject(SYSTEM_FONT);
...
static LRESULT REBAR_NotifyFormat(REBAR_INFO *infoPtr, LPARAM cmd)
{
    INT i;
 ...
 *
 *****************************************************/
static inline LRESULT REBAR_SetRedraw(REBAR_INFO *infoPtr, BOOL redraw)
{
...
static LRESULT WINAPI REBAR_WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        case WM_WININICHANGE:
            FIXME("WM %04x IS UNIMPLEMENTED.", uMsg);
            return DefWindowProcW(hwnd, uMsg, wParam, lParam);
